...
- Post-recording interactive flow: folder picker → note generation via `llm.py`
...
**Recording-loop robustness (record.py):**
- Wispr Flow only writes a `History` row *after* a recording stops and is processed — there is no in-progress row during a chunk. The polling loop therefore cannot use "no transcript yet" as a real-time health signal; the only authoritative failure detector is the post-stop `PROCESS_TIMEOUT`.
- **Backup nudge** (`BACKUP_NUDGE_DELAY = 1.5s`): after dispatching `wispr-flow://start-hands-free`, the loop silently re-sends the URL once at +1.5s. This covers the one real failure mode (Wispr occasionally drops the first start URL). No further nudges or warnings during the chunk — the old per-chunk retry timer produced false "Wispr may not be recording" warnings on every healthy chunk and has been removed.
- **Health check** (`HEALTH_CHECK_INTERVAL = 30s`): periodic `pgrep -f "Wispr Flow"` during a chunk. If Wispr crashes mid-chunk, the loop short-circuits and retries in seconds rather than waiting the full `RECORD_DURATION + PROCESS_TIMEOUT`.
- **Per-chunk retries** (`MAX_CHUNK_ATTEMPTS = 4`): if a chunk produces no transcript after stop, the chunk is auto-retried. On final failure, the last `History` row (timestamp / status / word count) is logged via `latest_history_row()` for debuggability.
- **Circuit breaker** (`MAX_CONSECUTIVE_CHUNK_FAILURES = 3`): if 3 chunks fail back-to-back, the session aborts cleanly instead of looping until `MAX_DURATION` (2h). The counter resets on any successful chunk, so transient hiccups don't trip it.
- **Timestamp format**: `get_utc_now()` emits Wispr-compatible 3-digit-ms timestamps (matching the `History.timestamp` column) with a 2-second backstop for clock skew, so SQL string-lex `>=` comparisons can never miss our own row.
...
- `install.sh` — Interactive installer: checks Wispr Flow, bootstraps uv, creates an isolated Python environment, configures `.env`, and installs the `wispr` command
- `get.sh` — Safe remote installer/updater that preserves `.env` and customized prompts
...
|---|---|---|
| `LLM_MODEL` | `gemini/gemini-3.1-pro-preview` | Model for note generation (any litellm-supported model) |
